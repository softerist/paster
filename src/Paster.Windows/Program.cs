...
namespace Paster.Windows
{
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            bool owns; using (Mutex mutex = new Mutex(true, "Local\\Paster.SingleInstance", out owns)) { if (!owns) return; using (WindowsPlatform platform = new WindowsPlatform(config)) { TransferCoordinator coordinator = new TransferCoordinator(platform, new SystemClock(), config); if(!String.IsNullOrEmpty(platform.RegistrationError)) coordinator.ReportError(platform.RegistrationError); using (ApplicationContext app = new PasterApplicationContext(platform, coordinator, config, configPath)) Application.Run(app); } }
        }
    }
}
